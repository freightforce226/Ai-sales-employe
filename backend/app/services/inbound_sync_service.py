...
class InboundSyncService:
    ...
    async def _run_reply_detection(
        ...
        """
        Production-grade reply matching and filtering.
        Returns dictionary specifying match success and completion statuses.
        """
        ...
        # Find the active follow-up schedule item for this customer
        sched_res = await self.db.execute(text("""
            SELECT s.id, s.step_number, el.sent_at, s.campaign_id
            FROM follow_up_schedule s
            LEFT JOIN email_log el ON s.source_email_log_id = el.id
        ...
        step_number = sched_row[1]
        source_sent_at = sched_row[2] or datetime.now(timezone.utc)
        campaign_id = sched_row[3]

        # Prioritized Matching Rules
        matched = False
        reason = ""

        # Rule 1: conversationId match
        if conversation_id:
            conv_check = await self.db.execute(text("""
                SELECT 1 FROM email_log 
                WHERE customer_id = :cust_id 
                  AND direction = 'outbound' 
                  AND conversation_id = :conv_id
                LIMIT 1
            """), {"cust_id": customer_id, "conv_id": conversation_id})
            if conv_check.fetchone():
                matched = True
                reason = "Matched conversationId"

        # Rule 2: internetMessageId match
        if not matched and internet_message_id:
            msg_check = await self.db.execute(text("""
                SELECT 1 FROM email_log 
                WHERE customer_id = :cust_id 
                  AND direction = 'outbound' 
                  AND internet_message_id = :msg_id
                LIMIT 1
            """), {"cust_id": customer_id, "msg_id": internet_message_id})
            if msg_check.fetchone():
                matched = True
                reason = "Matched internetMessageId"

        # Rule 3: In-Reply-To match
        if not matched and in_reply_to:
            irt_check = await self.db.execute(text("""
                SELECT 1 FROM email_log 
                WHERE customer_id = :cust_id 
                  AND direction = 'outbound' 
                  AND internet_message_id = :irt
                LIMIT 1
            """), {"cust_id": customer_id, "irt": in_reply_to})
            if irt_check.fetchone():
                matched = True
                reason = "Matched In-Reply-To headers"

        # Rule 4: References match
        if not matched and references:
            # Check if outbound internetMessageId is contained in inbound references
            ref_check = await self.db.execute(text("""
                SELECT 1 FROM email_log 
                WHERE customer_id = :cust_id 
                  AND direction = 'outbound' 
                  AND :references LIKE '%' || internet_message_id || '%'
                LIMIT 1
            """), {"cust_id": customer_id, "references": references})
            if ref_check.fetchone():
                matched = True
                reason = "Matched References headers"

        # Rule 5: Sender Email match
        if not matched and from_email:
            matched = True
            reason = "Matched Sender Email"

        # Rule 6: Timestamp fallback verification
        if matched and received_at <= source_sent_at:
            logger.info("Reply discarded because received timestamp is older than source sent date")
            return res_dict

        if matched:
            logger.info("Reply matching success", reason=reason, schedule_id=str(schedule_id))
